...
if strcmpi(sExperiment, 'drawSeries')
...
elseif strcmpi(sExperiment, 'drawSeries4')
    
    runThisRow = find(runOrder(:,2)==min(runOrder(:,2)),1);
    runOrder(runThisRow,2)=runOrder(runThisRow,2)+1;
...
for j = 1:size(expParameters,2)
    ...
    elseif strcmpi(expLvlRec{1,j}, 'CandleColour') && (strcmpi(sExperiment,'drawSeries2') || strcmpi(sExperiment,'drawSeries3') || strcmpi(sExperiment,'drawSeries4') || strcmpi(sExperiment,'glyphLearning3')) % added March 3 2017
        ...
        if rowAdapter == 1
            ...
            if strcmpi(sExperiment, 'drawSeries4')
                spreadFactor = 1;
            end
        elseif rowAdapter == 2
